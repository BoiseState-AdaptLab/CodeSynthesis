...
#include <sstream>
#define MAX_LEVEL 5
template <typename T, typename L> class BucketVector;
...
class BucketVector{
   L linearization;
   uint32_t binarySearch(const std::vector<T>& v, uint32_t val);

public:
   BucketVector(L linearization): linearization(linearization)
}
